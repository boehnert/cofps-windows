    ...
    def main(windList):
        try:
            ...
            for f in windList:
                ...
                simDir = dirName.replace("/data","E:")
                lastBit = baseName.split("_")[3]
                lastBitTime = lastBit.split(".")[0]
                newWindName = "S_D_" + lastBitTime +".shp"
                windPoints = os.path.join(simDir,newWindName)
                print windPoints
                if os.path.isfile(windPoints):
                    print "it exists"
                else:
                    outSpeedShape = createPoints(baseName, simDir, "speed")
                    arcpy.MakeFeatureLayer_management(outSpeedShape, "speed_layer")

                    lastBit = baseName.split("_")[3]
                    dirTiff = "web_WD10_Met_" + lastBit
                    
                    dirFullTiff = os.path.join(simDir, dirTiff)
                    outDirShape = createPoints(dirTiff, simDir, "direction")
                    ...
                    print "create file %s" %windPoints
                    arcpy.CopyFeatures_management("speed_layer", windPoints)
