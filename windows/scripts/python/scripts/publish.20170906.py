...
def isRecent(dt_str):
    """determines if input time string is recent"""
    if dt_str == None:
        return 0
    ...
    while (1):
        ...
        for d in docs:
            try:
                ...
                completedOn = d[cf.Dbt_complete]
            except:
                log(lp, "error getting document entries")
                log(lp, "(for sim_id %s)" % (sim_id))
                continue
            
            # process actively running or recently completed sims        
            if status == cf.Status_running:
                pass
            elif status == cf.Status_completed and isRecent(completedOn):
                pass
            else:
                continue
